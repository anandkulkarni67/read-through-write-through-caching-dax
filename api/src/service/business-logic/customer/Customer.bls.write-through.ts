...
class CustomerServiceWriteThrough {
    ...
    public async addCustomer(metadata: CreateCustomerMetadata): Promise<GetCustomerMetadata> {
        try {
            ...
            customerDataCache.set(customerMetadata.customerId, customerMetadata, process.env.IN_MEMORY_CACHE_TTL_SECONDS);
...
export const customerServiceWriteThrough = new CustomerServiceWriteThrough();